select 	gti.GenTreeId AS GT, 
		...
		/*Marriage Date*/
		SUBSTRING (STUFF(( SELECT ',' + cast(Marriage.PeriodStartDate as varchar(100)) FROM dbo.GenTreePeriod Marriage where gti.GenTreeId=Marriage.GenTreeId and gti.IndividualId=Marriage.IndividualId and Marriage.PeriodTypeCode= 7 order by Marriage.PeriodStartDate for XML PATH(''),Type).value('.','NVARCHAR(MAX)'),1,0,''),2) AS MSD,
		SUBSTRING (STUFF(( SELECT ',' + cast(Marriage.PeriodEndDate as varchar(100)) FROM dbo.GenTreePeriod Marriage where gti.GenTreeId=Marriage.GenTreeId and gti.IndividualId=Marriage.IndividualId and Marriage.PeriodTypeCode= 7 order by Marriage.PeriodStartDate for XML PATH(''),Type).value('.','NVARCHAR(MAX)'),1,0,'') AS MED
from GenTreeIndividuals gti
full join [dbo].[GenTree] gt on gt.GenTreeId=gti.GenTreeId
left join  
(select GenTreeId,IndividualId,PeriodTypeCode,PeriodStartDate, PeriodEndDate, row_number() over (partition by GenTreeId,IndividualId order by PeriodStartDate desc) as indrank from [dbo].[GenTreePeriod]) Birth
