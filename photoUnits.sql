...
SELECT  pic.*,
		...
		-- RelatedPictures
		STUFF(( SELECT cast(pu.PicId as varchar(max))  + ','FROM dbo.PicturesUnitPics pu with (nolock) where pu.PictureUnitId=pic.UnitId order by pu.PictureId for XML PATH(''),Type).value('.','NVARCHAR(MAX)'),1,0,'') PicId,
		STUFF(( SELECT cast(pu.PictureId as varchar(max)) + ',' FROM dbo.PicturesUnitPics pu with (nolock) where pu.PictureUnitId=pic.UnitId order by pu.PictureId for XML PATH(''),Type).value('.','NVARCHAR(MAX)'),1,0,'') PictureId,
